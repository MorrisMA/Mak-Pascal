...
SYMTAB_NODE_PTR formal_parm_list(int *, int *);
void reverse_list(SYMTAB_NODE_PTR *);
TYPE_STRUCT_PTR routine_call(SYMTAB_NODE_PTR, BOOLEAN);
TYPE_STRUCT_PTR declared_routine_call(SYMTAB_NODE_PTR, BOOLEAN);
void actual_parm_list(SYMTAB_NODE_PTR, BOOLEAN);
void block(SYMTAB_NODE_PTR);

extern void declarations(SYMTAB_NODE_PTR);
extern void init_symtab(void);
extern void enter_scope(SYMTAB_NODE_PTR);
extern void emit_program_prologue(void);
extern void emit_program_epilogue(SYMTAB_NODE_PTR);
extern void emit_main_prologue(void);
...
TOKEN_CODE follow_proc_id_list[] = {LPAREN, SEMICOLON, END_OF_FILE, 0};
    ...
    */
    ...
    if (new_level == old_level + 1) {
        ...
        */
        ...
        //fprintf(code_file, ";---\tsiz; lda STATIC_LINK,B;\n");
    ...
    //fprintf(code_file, ";---\tjsr abs;\n");
    //fprintf(code_file, ";---\tadj; -- restore caller stack pointer after Pascal routine\n");
                ...
                if (formal_parm_tp == real_typep) {
                    ...
                    */
                    if (actual_parm_tp == integer_typep) {
                        ...
                        //fprintf(code_file, ";---\tldy #$02; adj;\n");
                    }
                    emit_1(PUSH, reg(DX));  // ATOS = high word
                    //fprintf(code_file, ";---\tsiz; pha; rot;\n");
                    emit_1(PUSH, reg(AX));  // ANOS = low word 
                    //fprintf(code_file, ";---\tsiz; pha;\n");
                    ...
                    */
                    ...
                    //fprintf(code_file, ";---\tsiz; ldy #-$imm16; adj;          -- SP -= imm16;\n");
                    emit_2(MOVE, reg(DI), reg(SP));
                    //fprintf(code_file, ";---\tosx; oay; siz; txa;              -- tsy; MOVE DI,SP;\n");
                    emit_2(MOVE, reg(CX), integer_lit(size));
                    //fprintf(code_file, ";---\tsiz; lda #imm16;                 -- load CX\n");
                    emit_2(MOVE, reg(AX), reg(DS));
                    //fprintf(code_file, ";---\t---\n");
                    emit_2(MOVE, reg(ES), reg(AX));
                    //fprintf(code_file, ";---\t---\n");
                    emit_1(POP,  reg(SI));
                    //fprintf(code_file, ";---\toax; dup;                        -- save BP;\n");
                    //fprintf(code_file, ";---\tsiz; plx;                        -- POP SI;\n");
                    emit(MOVE_BLOCK);
                    //fprintf(code_file, ";---\tmvb #$33;                        -- block move\n");
                    //fprintf(code_file, ";---\toax; rot;                        -- restore BP;\n");
                } else {
                    emit_1(PUSH, reg(AX));
                    //fprintf(code_file, ";---\tsiz; pha;\n");
                }
