...
TYPE_STRUCT_PTR variable(SYMTAB_NODE_PTRp, USE);
	    ...
	    */
	    ...
	    if (   integer_operands(result_tp, tp2)
	        ...
	        */
            ...
            //fprintf(code_file, ";---\tldy #8; adj;\n");
	        emit_2(COMPARE, reg(AX), integer_lit(0));
            //fprintf(code_file, ";---\tsiz; cmp #0;\n");
            emit_2(MOVE, reg(AX), integer_lit(1));  /* default: load 1 */
            //fprintf(code_file, ";---\tphp; lda #1; plp;\n");
    ...
    if (saw_unary_op) {
	    if (base_type(result_tp) == integer_typep) {
            if (unary_op == MINUS) {
                ...
                //fprintf(code_file, ";---\tsiz; eor #$FFFF; siz; inc A;\n");
            }
	        ...
	        if (unary_op == MINUS) {
                ...
                //fprintf(code_file, ";---\tsiz; eor #$8000;     -- complement sign bit, msb ATOS\n");
		        ...
		        */
		        if (integer_operands(result_tp, tp2)) {
                    if (op == PLUS) {
                        ...
                        //fprintf(code_file, ";---\tclc; osx; siz; adc 0,B;              -- add ax,TOS\n");
                        //fprintf(code_file, ";---\tldy #2; adj;                         -- pop TOS\n");
                    } else {
		                emit_1(POP, reg(DX));                   // pop TOS
                        //fprintf(code_file, ";---\t---\n");
			            emit_2(SUBTRACT, reg(DX), reg(AX));     // sub TOS,ax
                        //fprintf(code_file, ";---\tsec; osx; siz; sbc 0,B;              -- sub ax,TOS\n");
                        //fprintf(code_file, ";---\tsiz; eor #$FFFF; siz; inc a;         -- neg ax\n");
                        //fprintf(code_file, ";---\tldy #2; adj;                         -- pop TOS \n");
	        ...
	        case OR: {
		        ...
		        */
		        if (boolean_operands(result_tp, tp2)) {
                    ...
                    //fprintf(code_file, ";---\tosx; siz; ora 0,B;\n");
                    //fprintf(code_file, ";---\tldy #2; adj;\n");
                } else
                    error(INCOMPATIBLE_TYPES);
	        ...
	        case AND: {
		        ...
		        */
		        if (boolean_operands(result_tp, tp2)) {
                    ...
                    //fprintf(code_file, ";---\tosx; siz; anl 0,B;\n");
                    //fprintf(code_file, ";---\tldy #2; adj;\n");
                } else
                    error(INCOMPATIBLE_TYPES);
	    ...
	    */
	    ...
	    if (   (use      != TARGET_USE )
            ...
            if (tp == char_typep) {
                ...
                //fprintf(code_file, ";---\tosx; lda 0,B\n");
                //fprintf(code_file, ";---\tldy #2; adj;\n");
            } else if (tp == real_typep) {
		        emit_2(MOVE, reg(AX), word_indirect(BX));
                //fprintf(code_file, ";---\tosx; siz; lda 0,B\n");
		        emit_2(MOVE, reg(DX), high_dword_indirect(BX));
                //fprintf(code_file, ";---\tdup; osx; siz; lda 2,B\n");
                //fprintf(code_file, ";---\tldy #2; adj;\n");
            } else 
                emit_2(MOVE, reg(AX), word_indirect(BX));
                //fprintf(code_file, ";---\tosx; siz; lda 0,B\n");
	    ...
	    if (tp->form == ARRAY_FORM) {
	        ...
	        */
            ...
            if (min_index != 0) {
                ...
                if (min_index == 1) {
                    //fprintf(code_file, ";---\tsiz; dec A;\n");
                } else {
                    //fprintf(code_file, ";---\tsec; siz; sbc #min_index\n");
                }
            }
	        ...
	        if (elmt_size > 1) {
		        ...
		        emit_1(MULTIPLY, reg(DX));
                switch(elmt_size) {
                    case 32: //fprintf(code_file, ";---\tsiz; asl A;\n");
                    case 16: //fprintf(code_file, ";---\tsiz; asl A;\n");
                    case 8 : //fprintf(code_file, ";---\tsiz; asl A;\n");
                    case 4 : //fprintf(code_file, ";---\tsiz; asl A;\n");
                    case 2 : //fprintf(code_file, ";---\tsiz; asl A;\n");
                             break;
                    default: //fprintf(code_file, ";---\tdup;                   -- input ANOS(AX)\n");
                             //fprintf(code_file, ";---\tsiz; lda #%d;          -- input ATOS(DX)\n", elmt_size);
                             //fprintf(code_file, ";---\tjsr _imul;             -- return DX:AX\n");
                             //fprintf(code_file, ";---\trot;                   -- keep AX, dump DX\n");
                             break;
                }
	        }
	        emit_1(POP,  reg(DX));
            ...
            //fprintf(code_file, ";---\tclc; osx; siz; adc 0,B;\n");
	        emit_1(PUSH, reg(DX));
            //fprintf(code_file, ";---\tosx; siz; sta 0,B;\n");
    ...
    if ((token == IDENTIFIER) && (tp->form == RECORD_FORM)) {
	    ...
	    */
	    ...
	    if (field_idp != NULL) {
            ...
            //fprintf(code_file, ";---\tclc; siz; adc #imm16;\n");
	        emit_1(PUSH, reg(AX));
