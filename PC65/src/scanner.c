...
void accumulate_value(float *, ERROR_CODE);
BOOLEAN token_in(TOKEN_CODE *);
...
/*--------------------------------------------------------------*/
/*  get_word        Extract a word token and down shift its     */
...
void get_number(void)
{
    ...
    */
    ...
    if (exponent != 0)
        nvalue *= (float) pow(10.0, exponent);
